...
function testMoney (str) {
  let regex = /^\$\d[\d{3}]*[,\.]?[\d{3}]*[,\.]?[\d{3}]*[,\.]?[\d{2}]*$/gmi
  //let regex = /^[\$\d]*[,\.]*[\d{3}]*[,\.]*[\d{3}]*(?:\.\d{2})$/gmi
  return regex.test(str)
}
...
function markDownLink (text) {
  let regex = /^\w{4}\:\/\/\w*\.\w{2,}\/?$/gmi
  /*return text.replace(regex, ("a href=" text "></a>"))*/
}
